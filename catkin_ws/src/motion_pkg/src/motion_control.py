...
def handle_motion(req):
    rospy.loginfo("Grasp attempt x{},y{},theta{}".format(req.x,req.y,req.theta))
    try:
        success = grasper.reset()
        assert  success   
    except:    
        rospy.logerr("Arm reset failed")
    grasp_pose = motion.compute_grasp(display_grasp=args.no_visualize)
    print("\n\n Grasp Pose: \n\n {} \n\n".format(grasp_pose))
...
def main():
    ...
    motion = Motion()
    signal.signal(signal.SIGINT, motion.signal_handler)
